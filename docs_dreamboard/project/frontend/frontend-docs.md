...
- the dense foreground dots read too heavy at their extracted size, so the drawn radius is scaled by `dotRadiusScale(r / pitch)`: dots up to 0.16 pitch (about the median) keep their size, dots from 0.35 pitch (about the 98th percentile) are drawn at `LARGE_DOT_SCALE` = 0.75, and the factor ramps linearly in between, so a larger dot always stays larger. The skyline and the snowcap timing use the extracted radii. A flat 0.65 factor for every dot was tried and read too thin, because the faint far ridges lost their tone
- dots are drawn from cached circle sprites grouped by 12 alpha levels. Dots that have settled are baked once into an offscreen layer, so a frame blits that layer and redraws only the dots still in flight (at most about 5.9k of the 14.6k). The layer canvas is allocated only once the first dots settle and dropped for the final frame, so a hero waiting at progress 0 (slide 4 before it is visited) or showing the finished picture keeps no offscreen copy
